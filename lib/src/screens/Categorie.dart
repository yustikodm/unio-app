...
class _CategorieWidgetState extends State<CategorieWidget> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    if ((widget._category.name=="University") && (widget.routeArgument.id!=101))
    {
      getuniversity2();
    }
    if ((widget._category.name=="Majors") && (widget.routeArgument.id!=101))
    {
      getMajors2();
    }
    if ((widget._category.name=="Vendor") && (widget.routeArgument.id!=101))
    {
      getVendors2();
    }
    if ((widget._category.name=="Places to Live") && (widget.routeArgument.id!=101))
    {
      getPlacestoLive2();
    }
    if ((widget._category.name=="Scholarship") && (widget.routeArgument.id!=101))
    {
      getScholarship2();
    }
    ...
    for (var i=0;i<hasilquery.length;i++)
    {
      setState(() {
        utilitiesList.popularList.add(
            new Utilitie(
                hasilquery[i]['name'], hasilquery[i]['logo_src'], hasilquery[i]['type'], '-###',25, 130, 4.3, 12.1));
  ...
  void getPlacestoLive2() async {
    final response = await http.get(
      Uri.parse('https://primavisiglobalindo.net/unio/public/api/place-to-lives'),
      // Send authorization headers to the backend.
      headers: {HttpHeaders.authorizationHeader: "VsNYL8JE4Cstf8gb9LYCobuxYWzIo71bvUkIVYXXVUO4RtvuRxGYxa3TFzsaOeHxxf4PRY7MIhBPJBly4H9bckY5Qr44msAxc0l4"},
    );
    print(response.body);
  ...
  void getScholarship2() async {
    final response = await http.get(
      Uri.parse('https://primavisiglobalindo.net/unio/public/api/university-scholarships'),
      // Send authorization headers to the backend.
      headers: {HttpHeaders.authorizationHeader: "VsNYL8JE4Cstf8gb9LYCobuxYWzIo71bvUkIVYXXVUO4RtvuRxGYxa3TFzsaOeHxxf4PRY7MIhBPJBly4H9bckY5Qr44msAxc0l4"},
    );
    print(response.body);
    ...
    for (var i=0;i<hasilquery.length;i++)
    {
      setState(() {
        utilitiesList.popularList.add(
            new Utilitie(
                "Scholarship "+hasilquery[i]['university']['name'], hasilquery[i]['picture'], hasilquery[i]['year'].toString(), '-###',25, 130, 4.3, 12.1));
        widget._category.utilities.add(
            new Utilitie(
                "Scholarship "+hasilquery[i]['university']['name'], hasilquery[i]['picture'], hasilquery[i]['year'].toString(), '-###',25, 130, 4.3, 12.1));
  ...
  void getmajors() async{
    final response = await http.get(
      Uri.parse('https://primavisiglobalindo.net/unio/public/api/university-majors?page='+halaman.toString()),
      // Send authorization headers to the backend.
      headers: {HttpHeaders.authorizationHeader: "VsNYL8JE4Cstf8gb9LYCobuxYWzIo71bvUkIVYXXVUO4RtvuRxGYxa3TFzsaOeHxxf4PRY7MIhBPJBly4H9bckY5Qr44msAxc0l4"},
    );
        ...
        SliverList(
          delegate: SliverChildListDelegate([
            Offstage(
              ...
              child: Column(
                children: <Widget>[
                  BrandHomeTabWidget(category: widget._category,utilitiesList: utilitiesList,),
                ],
              ),
            ),
            Offstage(
